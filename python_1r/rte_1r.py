...
delta_tu = np.zeros((2, lx, ly, lz))                            # delta opacity
exp_delta_tu = np.zeros((2, lx, ly, lz))
tu_1ray = np.zeros((lx, ly, lz))                                # opacity
x_tu1 = np.zeros((ly, lz))                                                      # xindex of tu = 1
tu0 = 0.1    
...
for idir in range(2):
    ...
    for i in range(ista[idir], iend[idir] + step[idir], step[idir]):
        for j in range(0, ly):
            for k in range(0, lz):
                ...
                # delta_I[idir, i, j, k] = ((beta_down - beta_up * exp_delta_tu[idir, i, j, k]) * delta_tu[idir, i, j, k]) \
                #                          / \
                ...
                # print(idir, i, j, k, delta_I[idir, i, j, k])
    ...
    for i in range(lx-1, 0, -1):
        for j in range(0, ly):
            for k in range(0, lz):
                tu_1ray[i-1, j, k] = tu_1ray[i, j, k] + delta_tu[0, i, j, k]
...
for i in range(1, lx):
    for j in range(1, ly):
        for k in range(1, lz):
            ...
            fx_down = (I[1, i-1, j, k] - I[0, i-1, j, k]) * mux_1ray * pi * 2
...
##### save data #####
print('# save data #')
np.savez('/mnt/solar07a/c0287shimizu/work/data/d001/1Ray/python.npz', \
        qrad=Qrad, qrad_tu1=Qrad_tu1, intensity=I, tu_1ray=tu_1ray, x_tu1=x_tu1)
