subroutine rte_multiray(ro, te, op, x, y, z, in, x_tu1, qrad)
!-------------------------------------------------------------------------------------------------------!
!!!----- def & init -----!!!
    ! rte multiray def
    use rte_mr_def
...
!!!----- intensity -----!!!
    ...
    do kdir = 1, 2
        ...
        do k = ksta(kdir), kend(kdir), step(kdir)
            ...
            exp_delta_tu(m, idir, jdir, kdir, i, j, k) = exp(- delta_tu(m, idir, jdir, kdir, i, j, k))
            !rte_exp(- delta_tu(m, idir, jdir, kdir, i, j, k))
...
!!!----- radiative heating rate -----!!!
    ...
    do jdir = 1, 2
    do kdir = 1, 2
        fx(i, j, k) = fx(i, j, k) + (4.0d0 * pi * omr(m) * step(idir) * in(m, idir, jdir, kdir, i, j, k))
        fy(i, j, k) = fy(i, j, k) + (4.0d0 * pi * omr(m) * step(jdir) * in(m, idir, jdir, kdir, i, j, k))
        fz(i, j, k) = fz(i, j, k) + (4.0d0 * pi * omr(m) * step(kdir) * in(m, idir, jdir, kdir, i, j, k))
    ...
    do k = 2, lz
        ...
        fy_up   = (fy(i,   j, k) + fy(i,   j-1, k) + fy(i,   j, k-1) + fy(i,   j-1, k-1)) / 4
        fy_down = (fy(i-1, j, k) + fy(i-1, j-1, k) + fy(i-1, j, k-1) + fy(i-1, j-1, k-1)) / 4

        fz_up   = (fz(i,   j, k) + fz(i,   j-1, k) + fz(i,   j, k-1) + fz(i,   j-1, k-1)) / 4
        fz_down = (fz(i-1, j, k) + fz(i-1, j-1, k) + fz(i-1, j, k-1) + fz(i-1, j-1, k-1)) / 4
        ...
        qrad_j = 4.0d0 * pi * alpha_mhd(i, j-1, k-1) * (j_mhd - beta_mhd(i, j-1, k-1))


        ! qrad
        tu_norm = rte_exp(- (tu_1ray(i, j, k) / tu0))
        !exp(- (tu_1ray(i, j, k) / tu0))
